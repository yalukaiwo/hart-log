...
>(({ children, className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className="flex items-start justify-between gap-x-4 border-b border-slate-200 pb-4 dark:border-slate-900"
      {...props}
    >
      ...
      <DrawerPrimitives.Close asChild>
        <Button
          variant="ghost"
          className="aspect-square p-1 hover:bg-slate-100 dark:hover:bg-slate-400/10"
        >
...
>(({ className, ...props }, forwardedRef) => {
  return (
    <DrawerPrimitives.Description
      ref={forwardedRef}
      className={cx("text-slate-500 dark:text-slate-500", className)}
      {...props}
    />
  );
});
...
const DrawerFooter = ({
  ...
  return (
    <div
      className={cx(
        "flex flex-col-reverse border-t border-slate-200 pt-4 sm:flex-row sm:justify-end sm:space-x-2 dark:border-slate-900",
        className
      )}
      {...props}
    />
